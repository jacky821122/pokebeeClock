...
- 2026-07-29 — 計時人員取消單班 4hr 上限，改為只設日上限 8hr；原本「normIn < 14:00 且 normOut >= 17:00 → 拆成兩筆缺卡 0hr」改為照算 + 單筆 raw span >= 7hr 加 note 提醒（避免真的連上一整天被歸零，同時保留漏打卡的警示）。核心概念改為「員工在正確時間打卡，算出來就是實際時數」，不再受營業/休息時間反推。規則以記錄日期分版本（`DAILY_CAP_ONLY_FROM = 2026-08-01`），之前的月份維持舊算法以確保已給薪資料可重現 (2b3e03b)
