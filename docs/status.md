...
- 2026-04-27 — 訊息系統補強：每次 show 結束時寫一列到 `message_responses`（有回應寫 emoji，無回應寫 ""，一列對應一次曝光）；每員工最近 3 則去重避免短時間重複（pool 太小時 fallback）；`/admin` 新增「訊息成效」表格（曝光 / 各 emoji 計數 / 回覆率）；移除 message POST 的 preview BYPASS；卡片標題「老闆的話」拿掉避免員工/老闆隔閡感
