...
export function resolveDeviceLabel(label: string | null): DeviceCheck {
  // BYPASS: feature/records-hour-contribution preview convenience. Remove before merging.
  if (process.env.NEXT_PUBLIC_BYPASS_AUTH === "1") return { ok: true, label: label ?? "" };
  if (label === null) {
    return {
      ok: false,
...
export async function checkDevice(req: NextRequest): Promise<DeviceCheck> {
  // BYPASS: feature/records-hour-contribution preview convenience. Remove before merging.
  if (process.env.NEXT_PUBLIC_BYPASS_AUTH === "1") return { ok: true, label: "" };
